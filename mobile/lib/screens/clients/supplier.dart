...
class _SupplierPageState extends State<SupplierPage> {
  ...
  Future<void> _loadSuppliers() async {
    final data = await CustomerService.getClients(clientType: 'Supplier');
    if (mounted) {
      setState(() {
        _suppliers = data;
        _loading = false;
  ...
  void _openEditModal(Map<String, dynamic> client) {
    showModalBottomSheet(
      ...
      builder:
          (_) => _EditSupplierSheet(client: client, onUpdated: _loadSuppliers),
    );
  }
          ...
          child:
              _suppliers.isEmpty
                  ...
                  : PaginatedDataTable2(
                    ...
                    columnSpacing: 15,
                    rowsPerPage:
                        _suppliers.length > 10 ? 10 : _suppliers.length,
                    ...
                    source: SupplierDataSource(_suppliers, _openEditModal),
                  ),
        ),
      ),
    );
...
class SupplierDataSource extends DataTableSource {
  ...
  SupplierDataSource(this._data, this.onEdit);
...
class _EditSupplierSheet extends StatefulWidget {
  ...
  const _EditSupplierSheet({required this.client, required this.onUpdated});
